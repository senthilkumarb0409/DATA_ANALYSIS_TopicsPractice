...
Select
*,
Case When Segs = 1 Then 'High Sales'
     When Segs = 2 Then 'Medium Sales'
	 When Segs = 3 Then 'Low Sales'
End SalesSegs
From
(
	Select 
		 OrderID,
		 Sales,
		 Ntile(3) over(Order by Sales Desc) Segs
	From Sales.Orders
)t


--Inorder to export the data, divide the orders into two grps
